        ...
        when (val s = state) {
            ...
            is UiState.Success -> {
                val filtered = s.data.filter { zone ->
                    ...
                    val factionMatch = factionFilter == null || run {
                        val f = zone.faction.lowercase()
                        val filter = factionFilter.lowercase()
                        f.contains(filter) ||
                            (filter == "alliance" && f.contains("alianza")) ||
                            (filter == "horde" && f.contains("horda"))
                    }
