...
@Composable
fun ProfessionListScreen(
    container: AppContainer,
    onProfessionClick: (Long) -> Unit,
    onBack: () -> Unit,
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text("Profesiones", style = MaterialTheme.typography.titleLarge) },
                navigationIcon = {
        ...
        when (val s = state) {
            ...
            is UiState.Error -> ErrorScreen(
                message = s.message,
                onRetry = { viewModel.loadProfessions() },
            ...
            is UiState.Success -> {
                LazyColumn(
                    ...
                    item {
                        HeroHeader(
                            title = "Profesiones",
                            subtitle = "${s.data.size} profesiones disponibles",
                            backgroundBrush = ThemeBrushes.quests,
                            imageRes = R.drawable.img_hero_home,
                            height = 140.dp
                        )
                    }
                    items(s.data, key = { it.id }) { profession ->
                        WowCardEnhanced(
                            title = profession.name,
                            subtitle = profession.typeLabel,
                            backgroundBrush = ThemeBrushes.quests,
                            imageRes = professionImageRes(profession.name),
                            faction = "Neutral",
